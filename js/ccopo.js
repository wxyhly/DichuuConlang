window.addEventListener("load", e => {
    if (!window.location.pathname.includes("ccopo")) {
        const iframe = document.getElementsByName("iframe_a").item(0);
        ...
        window.addEventListener("hashchange", () => {
            ...
            document.querySelectorAll(".left-panel a").forEach(a => {
                if (a.href.endsWith(window.location.hash.slice(1)))
                    a.classList.add("high-lighted");
            });
        });
    ...
    const hashed_data = Object.fromEntries(Object.values(raw_data).map(e => [e.unicode, { latin: e.latin, phonetique: e.phonetique }]));
    function char2latin(char) {
        return hashed_data[char.charCodeAt(0)]?.latin.replaceAll("-", "") ?? char;
    }
    function addRubyForStr(str) {
        let s = "";
        let rt = "";
    ...
    divChoice.forEach(div => {
        ...
        function nextQuestion() {
            ...
            if (!isFinite(id)) {
                div.lastElementChild.innerHTML = `已答对：${order.length}/${order.length} - 恭喜你完成了该练习所有题目！`;
                return;
            }
            div.innerHTML = "";
